...
function thread1()
{
    while true; do $CLICKHOUSE_CLIENT --query "RENAME TABLE alter_table TO rename_alter_table"; done
}

function thread2()
{
    while true; do $CLICKHOUSE_CLIENT -n --query "RENAME TABLE rename_alter_table TO alter_table"; done
}
