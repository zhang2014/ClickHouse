...
CREATE TABLE test.compression_codec(id UInt64 CODEC(LZ4), data String CODEC(ZSTD), ddd Date CODEC(NONE), somenum Float64 CODEC(ZSTD(2)), fff UInt64 CODEC(Delta(UInt64))) ENGINE = MergeTree() ORDER BY tuple();

INSERT INTO test.compression_codec VALUES(1, 'hello', toDate('2018-12-14'), 1.1, 1);
INSERT INTO test.compression_codec VALUES(2, 'world', toDate('2018-12-15'), 2.2, 2);
INSERT INTO test.compression_codec VALUES(3, '!', toDate('2018-12-16'), 3.3, 3);

SELECT * FROM test.compression_codec ORDER BY id;

OPTIMIZE TABLE test.compression_codec FINAL;

INSERT INTO test.compression_codec VALUES(2, '', toDate('2018-12-13'), 4.4, 4);
...
ALTER TABLE test.alter_compression_codec ADD COLUMN data_one String DEFAULT 'default_one';
ALTER TABLE test.alter_compression_codec ADD COLUMN data_two String DEFAULT 'default_two' CODEC(ZSTD);
