...
def test_dynamic_query_handler_with_params_and_settings():
    with contextlib.closing(SimpleCluster(ClickHouseCluster(__file__), "dynamic_params_and_settings", "test_and_settings_dynamic")) as cluster:
...
def test_other_configs():
    with contextlib.closing(SimpleCluster(ClickHouseCluster(__file__), "other_tests", "other_tests")) as cluster:
