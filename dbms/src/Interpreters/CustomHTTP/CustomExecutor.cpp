...
{
    size_t type_name_end_pos = matcher_or_executor_type_name.find('[');
...
CustomExecutorPtr CustomExecutors::createCustomExecutor(const Configuration & config, const String & config_prefix)
{
    ...
    for (const auto & type_name : matchers_or_query_executors_type)
    {
        String matcher_or_query_executor_type = fixMatcherOrExecutorTypeName(type_name);
        ...
        if (matcher_creator_it != custom_matcher_creators.end())
            query_matchers.push_back(matcher_creator_it->second(config, config_prefix + "." + matcher_or_query_executor_type));

        if (query_executor_creator_it != query_executor_creators.end())
            query_executors.push_back(query_executor_creator_it->second(config, config_prefix + "." + matcher_or_query_executor_type));
    }

    checkCustomMatchersAndQueryExecutors(query_matchers, query_executors);
    return std::make_shared<CustomExecutor>(query_matchers, query_executors);
}

void CustomExecutors::checkCustomMatchersAndQueryExecutors(std::vector<QueryMatcherPtr> & matchers, std::vector<QueryExecutorPtr> & query_executors)
{
