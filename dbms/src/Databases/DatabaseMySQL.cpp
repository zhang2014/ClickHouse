...
ASTPtr DatabaseMySQL::getCreateDatabaseQuery(const Context &) const
{
    ...
    create_query->setDatabase(std::make_shared<ASTIdentifier>(database_name));
    ...
    const String & table_name, const NamesAndTypesList & columns_name_and_type, const UInt64 & table_modification_time) const
{
    ...
    create_table_query->setTable(std::make_shared<ASTIdentifier>(table_name));
    create_table_query->setDatabase(std::make_shared<ASTIdentifier>(database_name));
